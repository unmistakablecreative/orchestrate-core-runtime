...
def is_protected_file(filename):
    protected_names = {
        'system_settings.ndjson', 'command_center.json',
...
def create_code_blueprint(params):
    ...
    if is_protected_file(filename):
        return {'status': 'error', 'message': '❌ This file is protected and cannot be modified.'}
    output_py_filename = filename.replace('.json', '.py')
    internal_py_path = os.path.abspath(os.path.join('..', 'Orchestrate_Hackathon', 'tools', output_py_filename))
    blueprint = {
        'filename': internal_py_path,
        'imports': imports,
        'functions': {},
        'router': {}
    }
    ...
    return {'status': 'success', 'message': f'✅ Created code blueprint at {filepath}'}
...
def add_function_to_blueprint(params):
    ...
    return {'status': 'success', 'message': f"✅ Function '{function_name}' added to {filename}."}
...
def update_function_in_blueprint_file(params):
    ...
    if function_name not in data['functions']:
        return {'status': 'error', 'message': f"⚠️ Function '{function_name}' not found."}
    data['functions'][function_name] = {'params': function_params, 'body': body}
    with open(path, 'w') as f:
        json.dump(data, f, indent=4)
    return {'status': 'success', 'message': f"✅ Function '{function_name}' updated in {filename}."}
...
def remove_function_from_blueprint(params):
    ...
    with open(path, 'w') as f:
        json.dump(data, f, indent=4)
    return {'status': 'success', 'message': f"🗑️ Function '{function_name}' removed from {filename}."}
...
def read_function_from_blueprint(params):
    ...
    return {
        ...
        'function': {
            'name': fn,
            'params': data['functions'][fn]['params'],
            'body': data['functions'][fn]['body']
        }
    }
...
def list_functions_in_blueprint(params):
    ...
    with open(path, 'r') as f:
        data = json.load(f)
    return {'status': 'success', 'functions': list(data.get('functions', {}).keys())}
...
def configure_router_in_blueprint(params):
    ...
    with open(path, 'w') as f:
        json.dump(data, f, indent=4)
    return {'status': 'success', 'message': f'✅ Router configured in {filename}.'}
...
def compile_blueprint_to_script_file(params):
    ...
    output_path = os.path.abspath(os.path.join('..', 'Orchestrate_Hackathon', 'tools', data['filename']))
...
def add_function_to_code_file(params):
    ...
    try:
        ...
        # Find the line before which we insert (before Action Router or if __name__)
        insert_index = None
        ...
        return {"status": "error", "message": f"❌ Failed to add function: {str(e)}"}
        ...
        def visit_FunctionDef(self, node):
            ...
            if node.name == function_name:
                ...
                new_func = ast.FunctionDef(
                    name=function_name,
                    args=ast.arguments(posonlyargs=[], args=[ast.arg(arg=p) for p in param_list], kwonlyargs=[], kw_defaults=[], defaults=[]),
                    body=body_ast,
                    decorator_list=[]
                )
                patch_applied = True
...
def rename_function_in_file(params):
    ...
    if f"def {old_name}(" not in content:
        return {"status": "error", "message": f"❌ Function '{old_name}' not found."}
    ...
    return {"status": "success", "message": f"✅ Renamed '{old_name}' to '{new_name}' in {filename}"}
...
def replace_in_code_file(params):
    ...
    try:
        ...
        if old_text not in content:
            return {"status": "error", "message": "❌ Old text not found in file."}
...
def replace_tail_from_line(params):
    ...
    try:
        if start_pattern.strip() != 'if __name__ == "__main__":':
            return {'status': 'error', 'message': 'replace_tail_from_line may only be used to replace __main__ blocks.'}
        ...
        for i, line in enumerate(lines):
            ...
            return {'status': 'error', 'message': f"Pattern '{start_pattern}' not found."}
...
def add_endpoint_to_fastapi_file(params):
    ...
    if any(f'def {function_name}(' in line for line in lines):
        return {'status': 'error', 'message': f"❌ Function '{function_name}' already exists."}
    insert_idx = 0
    for i, line in enumerate(lines):
    ...
    return {'status': 'success', 'message': f"✅ Endpoint '{route}' inserted via {method.upper()} method."}
...
def remove_function_from_code_file(params):
    ...
    with open(filename, 'w') as f:
        f.writelines(new_lines)
    return {'status': 'success', 'message': f"🗑️ Function '{function_name}' removed from {filename} (without nuking the file)."}
    ...
    try:
        ...
        class ActionMapUpdater(ast.NodeTransformer):
            def visit_Assign(self, node):
                if isinstance(node.targets[0], ast.Name) and node.targets[0].id == "action_map":
                    ...
                    if action_name in keys:
                        raise ValueError(f"⚠️ Action '{action_name}' already exists.")
                    node.value.keys.append(ast.Str(s=action_name))
                    node.value.values.append(ast.Name(id=function_name, ctx=ast.Load()))
                return node
        ...
        return {"status": "success", "message": f"✅ Inserted: '{action_name}': {function_name}"}

    except ValueError as ve:
        return {"status": "error", "message": str(ve)}
    except Exception as e:
        return {"status": "error", "message": f"❌ Failed to update action_map: {str(e)}"}
